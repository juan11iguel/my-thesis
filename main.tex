...
\begin{kaobox}[title=asd what is going on,colback=MediumPurple2!25!white,colbacktitle=MediumPurple2!25!white]
    What is thiiiis
    \begin{enumerate}
      \item adjust
      \item asdasd
...
\marginannotation{Process \textit{vs} waste heat definition of efficiency}{
	In an efficient process heat driven system, between two plants that
	produce the same amount of useful product, the most efficient one is the
	one that uses the least amount of external heat.
	While given two plants that consume the same amount of waste
	heat, the most efficient one is the one that produces more product given
	that heat.
}
