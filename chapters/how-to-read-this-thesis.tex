...
\tldrbox{ This preliminary chapter explains how to read this document, mainly
    why the large margins used throughout the manuscript, what is placed in
    them, the different boxed environments, and how to use the interactive
    features of the manuscript. This is an example of a \gls{tldrLabel} box. It
    contains an Abstract/Summary of the main point of the chapter and are placed
    at the beginning of every chapter. }

This \LaTeX{} template\sidenote{The
\href{https://github.com/fmarotta/kaobook/}{kaobook} class from Federico
Marotta} is designed with large margins, on the one hand this allows to have
shorter lines, which makes for an easier reading experience but most
interestingly, it also allows to place additional information in the margins,
such as side notes, side citations, figures\sidenote{See
\reffig{how-to-read:example-figure}}, tables\sidenote{See
\reftab{example}}\dots~your imagination is the limit! Or rather \LaTeX{}
compilation errors and your patience are. Throughout this manuscript I will add
side notes\sidenote{Like this one! They are like footnotes, but placed in the
margin of the page} to provide additional information and comments that would
otherwise be too distracting and verbose to include in the main text, constantly
interrupting the flow of the reading. The side notes are not essential to
understand the content of the document, but mostly complementary.

\section*{Boxed environments}

Both problem definition boxes (\eg~\refprob{example}) and model definition boxes
(\eg~\refmod{example}) are countered environments and can be referenced in the
text.

\begin{marginfigure}[*2]
    \includegraphics[]{bussines_cat.jpg}
    % \caption{Back view of the WCT}
    \savebox\captionqr{\qrcode[hyperlink,height=0.4in]
    {https://web.archive.org/web/20251014125656/https://knowyourmeme.com/memes/business-cat}}
    % {https://www.youtube.com/watch?v=sZkB11pO9R8}}
    % \caption{\parbox{0.8\textwidth}{Line 1 of caption\\Line 2 of caption}}
	\caption[Example figure]{Example figure. Try clicking or scanning the QR code to access the
	interactive version.\\[1ex]\usebox\captionqr}
    \labfig{how-to-read:example-figure}
\end{marginfigure}


\begin{problemcounter}{Problem definition box example}
...
\begin{modelcounter}{Model definition box example}
    \begin{equation*}
        out_{1},\,out_{2} = \text{some\:useful\:model}\:(in_{1},\, in_{2},\, in_{3})
    \end{equation*}
    \labmod{example}
\end{modelcounter}

\reminder{This is a reminder}{To remind the reader of something mentioned earlier in the manuscript relevant to the current context.}

\annotation{This is an annotation}{To provide additional explanation or commentary on a specific point in the text.}
...
reader to access the corresponding online resource\sidenote[][*-4]{
...
well as the source code of this document are hosted in a \href{\repositoryBaseUrl}{Zenodo repository}\sidenote[][*-1]{
    \qrcode[hyperlink,height=0.5in]{\repositoryBaseUrl} \\[2ex] \url{\repositoryBaseUrl}
