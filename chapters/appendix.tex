...
% Updated to use enthalpy instead of specific heat
\begin{align}
...
\reffig{solarmed:std:control-volume}. Any external stream entering the control
volume is considered an exergy input ($\dot{E}x_{in}$), while any stream leaving
it is considered an exergy output ($\dot{E}x_{out}$). A general expression to
...
equations~\sidecite[*-7]{pitzer_thermodynamics_1973}. 

Finally, in order to calculate the specific exergy flows, libraries in MATLAB
~\sidecite[*-7]{sharqawy_thermophysical_2010,nayar_thermophysical_2016} and
Python ~\sidecite[*-2]{romera_iapws_2021} are available. However, they are
...
tool~\sidecite[]{marcellos_pyequion_2021}.
...
Figures~\ref{fig:ap:med-model:effect}--\ref{fig:ap:med-model:preheater}) ensuring
consistent heat exchange and energy balances. The preheaters are solved similarly. Throughout the process, mass and energy conservation are
