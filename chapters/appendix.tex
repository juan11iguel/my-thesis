...
\begin{itemize}
    \item Effect heat transfer coefficients ($U_{ef}$)
    \item Preheater heat transfer coefficients ($U_{ph}$)
    \item Preheater-effect distribution line vapor losses ($\Delta M_v$) ...
\end{itemize}

\textbf{Plant parameters:}
\begin{itemize}
    \item Mixer valve positions ($Y$)
    \item Effect and preheater areas ($A_{ef,i}$, $A_{ph,i}$)
    \item Condenser area ($A_c$)
    \item Number of effects and preheaters ($N_{ef}$)
\end{itemize}

\textbf{Model inputs:}
\begin{itemize}
    \item Heat source flow rate and inlet temperature ($M_s$, $T_{s,in}$)
    \item Cooling water flow rate and inlet temperature ($M_c$, $T_{c,in}$)
    \item Feedwater flow rate and inlet temperature ($M_f$, $T_{f,in}$)
    \item Operating pressures per effect and preheater ($P_{ef,i}$, $P_{ph,i}$)
    \item Number of effects and preheaters ($N$)
    \item Feedwater salinity ($S_f$)
\end{itemize}
