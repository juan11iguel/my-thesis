...
From a literature review, works can be found that optimize the operation of the
cooling system. Of special interest are the works of
Marín~\sidecite{martin_optimal_2013,martin_optimal_2015} where a .... strategy
is proposed to optimize both the power block and the cooling system. In
\cite{martin_optimal_2013} a \gls{wctLabel} is analyzed, while on
\cite{martin_optimal_2015} a \gls{dcLabel} is used instead. Annual analysis is
performed (sampled monthly) and a techno-economic analysis is performed. The
results show ... 
It can be concluded that there are not many works focusing on the operation
optimization. From those that do, they study only one particular system, either
a dry cooler or a wet cooler, and not the combined cooling system. From those
do, they are focused on the design optimization side, not on the operation.
...
alternatives is analyzed in terms of their two main consumptions: electricity
and water. The optimization problems are formulated to minimize the cost of
cooling a thermal load, where this cost is made up by the two mentioned
consumptions. Even though in principle this methodology could be applied to any
application where cooling a thermal load is required, special focus is put on
modelling and considering the water resource availability, since it is a key
factor especially in solar thermal applications.\sidenote{See
\nrefch{intro:cooling}}
...
\section[Horizon optimization]{Horizon optimization}[Horizon optimization]
