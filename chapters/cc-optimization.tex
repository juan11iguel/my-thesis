...
\tldrbox{ This chapter describes optimization problems for a combined cooling
    ...
    They key challenge is to manage the limited water resource. From the studied
...
the first to optimize the year-round operation of a \gls{cspLabel} system not only
...
condenser pressure is reached, in which case the wet units activated. This
...
\begin{kaobox}[title=The Increasing Variability of Rainfall] 
    ...
    Source: Uncharted Waters: The New Economics of Water Scarcity and Variability~\cite{damania_uncharted_2017}
\end{kaobox}
...
\begin{enumerate}
    ...
    the associated price ($P_e$) is the pool market price. In other
    applications,this electricity must be purchased at the market price. 
    
    As for the water, two sources are considered: the water price from source 1
    is denoted as $P_{w,s1}$, and from source 2 as $P_{w,s2}$, being $P_{w,s1}<P_{w,s2}$.
    ...
    of them, the cheaper one coming from a dam/reservoir is limited in volume
...
Equations~\ref{cc:optimization:wa1}--\ref{cc:optimization:wa3}. The
...
distribution variables ($R_p$ and $R_s$). The environment remains unchanged from
the wet cooler case, as it still includes water-related variables.

\begin{figure}
    \includegraphics[width=.7\textwidth]{figures/cc-pareto_fronts_for_different_scenarios_and_power_and_hydraulic_dist_for_different_scenarios.png}
...
cooler\sidenote{Particularly for the pilot plant described in \refch{cc:facility}}
can meet a specific cooling load under four diverse scenarios: two  different
environment conditions and cooling loads. The
optimal operating points are evaluated in terms of the two consumptions: electricity
($C_e$) and water ($C_w$) and form the Pareto front\marginreminder{Pareto front}{
...
summer conditions. If the system operated only with the \gls{dcLabel}, the vapor
temperature (\ie turbine back-pressure) would rise, negatively impacting the
...
load (7.4~kW$_e$). By combining the \gls{dcLabel} with the \gls{wctLabel},
...
\begin{problemcounter}{\gls{ccLabel} - horizon}
    ...
    \begin{align*}
        ...
        & C_{w,s1,i} = \frac{\min(V_{avail,i}, C_{w,i} \cdot T_s)}{T_s} \\
...
Although the formulation in principle allows for an arbitrarily long prediction 
horizon, in practice it is limited to the number of steps for which reliable 
forecasts of the environmental variables are available. In this study, water 
availability is allocated on a daily basis; therefore, the prediction horizon 
begins at the current time when the optimization is launched and extends until 
the end of the operation day. 
...
operation would not be achieved for such a set of inputs\sidenote{Either the
...
one variable by analyzing how the complete model is solved and described in
...
layered, dynamic-weighted directed graph. Each layer corresponds to a time 
step in the prediction horizon, and each node within a layer represents a point 
on the corresponding Pareto front. The objective is to find a path 
\(\mathbf{p} = (p_1, p_2, \ldots, p_{n_{\text{steps}}})\), where \(p_i\) is the 
selected node at time step \(i\), that minimizes the total cumulative cost \(J\) 
along the path. The weight\sidenote{\ie the water cost component} on each layer is 
dynamic---depending on the trajectory taken to reach that layer. The problem can 
be formulated as:
...
The step cost contribution to the cumulative cost \(J_{i}\) depends on both the
consumptions (\ie electricity and water) of the node \(p_i\), as well as on a
dynamic cost function determined by the path history. In particular, the water
cost is correlated with the water resource availability
(\(J_{w,i}=f(V_{avail,i-1})\))\sidenote{See
Equations~\ref{eq:cc:optimization:wa1}--\ref{eq:cc:optimization:wa3}}, which is
itself a function of the previous decisions. This calculation is straightforward
and can be performed almost instantly, making it the only information required
to compute the step associated cooling cost:
...
As mentioned, costs (\(J_{w,i}\)) depend on prior path decisions,
this introduces path-dependency into the cost function, and makes the problem
non-trivial to solve via simple shortest path algorithms. The problem can be
handled via dynamic programming, graph search (like Dijkstra or A*), or
