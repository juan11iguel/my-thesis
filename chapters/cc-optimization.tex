...
In this chapter, the optimization of the operation of different cooling
alternatives are analyzed in terms of their two main consumptions: electricity
and water.

Even though in principle this methodology could be applied to any application
where cooling a thermal load is required, special focus is put on modelling and
considering the water resource availability, since it is a key factor
especially in solar thermal applications.\sidenote{See \nrefch{intro:cooling}}
...
The optimization problems are formulated to minimize the cost of cooling a
thermal load, which is composed of the cost of electricity ($J_e$) and the cost
of water ($J_w$).
...
optimization approach in \nrefsec{cc:optimization:shrinking-horizon} where the
...
\begin{description}
    ...
    \item[Thermal load] The thermal load is defined either by a vapor flow rate
...
From a process perspective this applies to all process variables, except for
...
\section[Shrinking horizon optimization]{Shrinking horizon optimization}[Horizon optimization]
...
variables that are direct inputs on the process\sidenote{Decision variables
...
solved, yielding a Pareto front. A global
optimization problem is then formulated to select a path through the
sequence of Pareto fronts, minimizing a cumulative objective (e.g., cost or
distance), akin to a pathfinding or \gls{tspLabel}-like problem over Pareto-optimal
points.
...
\begin{figure*}[h!]
    ...
    In step 2. three paths are illustrated: a water-greedy
...
conditions ($q_{wct}$, $T_{wct,in}$) can be determined by the second valve
...
only four values from which a feasible wet cooling tower fan speed exists, \ie
within its bounds $\omega_{wct}\,\in [\underline{w}_{wct}, \overline{w}_{wct}]$, thus greatly
...
and is illustrated in \reffig{cc:optimization:horizon-methodology}--2. Each
node represents a point in the Pareto front of a step, and edges represent the
transition costs between these points, that is, the cumulative cost so far
($J_{0..i}$). The goal is to find a path through the graph that minimizes the
final cumulative cost ($J_{0..n_{steps}}$ or simply \( J \)).

The transition cost is correlated to the current resource availability
($V_{avail,i}$)\sidenote{See
...
Three paths are illustrated in \reffig{cc:optimization:horizon-methodology}--2.
The
...
values of \(C_w\) from bottom to top.}, so in the first split it can be seen
it achieves the lowest cost of operation, but also leaves the least water
...
\textcolor[HTML]{FFCE9F}{solid-\textit{orangey} path (\texttt{--})} is a compromise
between the two, it uses water more efficiently, leaving no water available at
the end of the horizon and minimizing the overall cost of operation.

% Definición formal del problema
This subproblem is a combinatorial
optimization problem, in particular, a layered weighted directed graph and can
be mathematically defined as follows:

\begin{equation*}
    \min_{\mathbf{p}} \quad J = \sum_{i=1}^{n_{steps}} C_{transition,i} \cdot T_s
\end{equation*}

where \( \mathbf{p} \) is the path through the Pareto fronts, \( n_{steps} \)
is the number of steps in the prediction horizon, and \( C_{transition,i} \)
is the transition cost between the Pareto front points at step \( i \). The
transition cost is defined as the difference in cumulative cost between two
points in the Pareto front, which can be computed as follows:
\begin{equation*}
    C_{transition,i} = C_{e,j} + C_{w,j} - C_{e,k} - C_{w,k}
\end{equation*}
where \( j \) and \( k \) are the indices of the two points in the Pareto front
at step \( i \) that are being compared. The transition cost represents the
difference in cumulative cost between two points in the Pareto front, and it
is computed as the difference in electricity and water consumption between the
two points. This cost is computed for every pair of points in the Pareto front
at each step, resulting in a matrix of transition costs.
% Mencionar algoritmos candidatos para resolver el problema
