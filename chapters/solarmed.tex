...
\myepigraphhead[500]{Such arbitrariness had its origins in the example of the Catalan sage, for whom wisdom was not worthwhile if it could not be used to invent a new way of preparing chickpeas.}{Gabriel García Márquez, \textit{One Hundred Years of Solitude}}
...
that governs the autonomous operation of the coupled solar-\gls{medLabel} system. The
upper-level controller solves a mixed-integer nonlinear economic problem that
determines subsystem activation, startup, shutdown, and regulation while
exploiting solar availability and storage flexibility. When compared over a full
week of operation, the strategy outperforms both a heuristic rule-based baseline
and a continuous-only optimization approach by 32~\% and 21~\%, respectively.
These gains stem from the ability to maximize useful temperature differences and
align \gls{medLabel} operation with the temporal value of heat, achieving performance
levels comparable to systems optimized for waste-heat recovery.\\
...
(DEAHP)~\sidecite[*-7]{alarcon-padilla_application_2007}, both designed to increase
the gain output ratio\sidenote[][*-3]{A metric that measures the heat provided to the
...
discharge applications where \gls{roLabel} performance declines sharply.
...
option ---particularly for brine concentration and \gls{zldLabel}
