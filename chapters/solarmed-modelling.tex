...
\begin{tabular}{cll}
    ...
    \midrule
    0 & Off & $q_{sf}\land q_{ts,src}==0$ \\
    1 & Warming up \gls{sfLabel} & $q_{sf} > 0 \land q_{ts,src}==0$ \\
    2 & Recirculating \gls{tsLabel} & $q_{sf} ==0 \land q_{ts,src} > 0$ \\
    3 & \gls{sfLabel} heating up \gls{tsLabel} & $q_{sf}\land q_{ts,src} > 0$
    \\
    \bottomrule
\end{tabular}
}
...
\begin{tabular}{cll}
    ...
    \midrule
    0 & Off & $\forall q == 0$ \\
    1 & Generating vacuum & $med{vac} == 2$ \\
    2 & Idle & $\forall q == 0 \land med{vac} == 1$ \\
    3 & Starting-up & $\forall q > \underline{q} \land med{vac} \ge 1 \land \forall T >
    \underline{T} $ \\
    4 & Shutting down & $\exists$ q < \underline{q} \\
    5 & Active & $\forall q > \underline{q} \land med{vac} \ge 1 \land \forall T >
    \underline{T} $\\
    \bottomrule
\end{tabular}
}
