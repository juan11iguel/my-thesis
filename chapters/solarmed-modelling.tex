...
\begin{modelcounter}{Solar field}
    \begin{align*}
        ...
        & K_{1} = \beta / (\rho \cdot c_p \cdot A_{cs}) \tag*{{\small\textit{Solar contribution [K·m$^2$/J]}}} \\
        & K_{2} = H / (L_{pipe,eq} \cdot A_{cs} \cdot \rho \cdot c_p) \tag*{{\small\textit{Environment losses [1/s]}}} \\
        & K_{3} = 1 / (L_{pipe,eq} \cdot A_{cs} \cdot c_f) \cdot (1/3600) \tag*{{\small\textit{Heat absorbed [h/(3600·m$^3$·s)]}}} \\
...
considered. The model is a data driven one, specifically a \gls{gprLabel} model
calibrated using data from two experimental campaigns in the pilot plant: the
first one is described in Palenzuela et
al.~\sidecite{palenzuela_experimental_2016} and took place between 2009 and
2012. The second experimental campaign took place between 2021 and 2024 and is
the one described in \refch{solarmed:std}, not as extensive as the first one but
extending the operation range of the heat source temperature. In total, the
experimental data spans 6 years of operation and make available
549 operation points with the range per variable shown in \reftab{solarmed:modelling:med-data-range}. 
...
\begin{modelcounter}{SolarMED model}
    \begin{align*}
        q_{med,d},\,C_{e} &= f\bigl(
            ...
            &\qquad q_{ts,src}, q_{sf}, \text{med}_{vac,st}, T_{med,c,in}, T_{amb}, I;\,
            \theta_{sf}, \theta_{hx}, \theta_{med}, \theta^{fsm}_{sfts}, \theta^{fsm}_{med}, \theta_{\infty}
        \bigr) \\ \\
        ...
        &T_{\text{sf,out}} = \text{sf model}\!\left(
            ...
            \theta_{sf}
        \right) \\
        &\mathbf{T}_\text{ts,h},\; \mathbf{T}_\text{ts,c} = \text{ts model}\Bigl(
            \mathbf{T}_\text{ts,h}(k-1),\;
            \mathbf{T}_\text{ts,c}(k-1),\;
            T_\text{hx,s,out}, \\
            &\quad T_\text{med,s,out},\;
            q_\text{ts,src},\;
            q_\text{ts,dis},\;
            T_\text{amb};\;
            \boldsymbol{\theta}_\text{ts}
        \Bigr) \\
        &T_{sf,in},\,T_{hx,s,out} = \text{hx model}\!\left(
            T_{sf,out}, T_{ts,c,b}, q_{sf}, q_{ts,src}, T_{amb}; \theta_{hx}
        \right) \\
        &q_{ts,dis}=\text{3wv model}(q_{med,s}, T_{med,s,in}, T_{med,s,out}) \\
        &q_{med,d},\,T_{med,s,out},\,q_{med,c},\,T_{med,c,out} = \text{med model}\left( q_{med,s},q_{med,f}, T_{med,s,in}, T_{med,c,out}, T_{med,c,in} \right) \\ \\
        &C_{e,sf} = \text{sf electrical consumption}(q_{sf}) \\
        &C_{e,ts} = \text{ts electrical consumption}\left(q_{ts,src}\right) \\
        &C_{e,med} = \text{med electrical consumption}(q_{med,s}, q_{med,f}, q_{med,c}, q_{med,d}, q_{med,b}) \\
...
limit, or forced to zero if a component is inactive, or to a predefined
...
temperatures and the discharge flows. This set of dependencies between the
different subsystems is visualized in \reffig{solarmed:modelling:complete_model}.
