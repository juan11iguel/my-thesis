...
\tldrbox{ This chapter presents the discrete and complete dynamic modelling of
    the \gls{solarmedLabel} system.  First, continuous physics-based and
    data-driven models for the solar field, heat exchanger, thermal storage,
    three-way valve, and \gls{medLabel} plant.
    
    These dynamic models are then combined with the discrete behavior of the
    installation. This is represented by means of two supervisory
    \glspl{fsmLabel} that define the operation states of the heat generation and
    storage subsystem (\gls{sftsLabel}) and the multi-effect distillation
    subsystem (\gls{medfsmLabel}). Each \gls{fsmLabel} determines subsystem
    activation and transitions based on system inputs, internal rules, and
...
in Ampuño et al.~\sidecite{ampuno_modeling_2018} is used to model the solar
field. The model has two types of parameters: dynamic and constant\sidenote{Also
...
These two dynamic parameters are calibrated using experimental data which
together with the constant parameters can be found in
...
modelled~\sidecite[*-10]{ampuno_apparent_2019}\sidenote[][*-10]{Transport delays are a common
...
presented in Normey-Rico~\etal~\sidecite[*-10]{normey-rico_robust_1998} since delays
...
condenser between 10:30 and 12:30 in the 202303031 test ---it becomes difficult to
...
phase of the 202303031 test and throughout most of the 20230418 test.
...
\begin{figure*}[htbp]
    \includegraphics[width=\linewidth]{solarmed_fsm_evolution_20230703.png}
...
initialized provided with the initial discrete states (\eg vacuum generated,
brine emptied from final effect, etc) and system parameters (\eg thermal storage
volumes, heat exchanger transfer conductance, \gls{medfsmLabel} timings, etc).
Also need to be initialized the initial temperatures for the solar field and
thermal storage.
...
field\sidenote{\reffig{solarmed:modelling:validation:solarmed} -- \textit{Solar
...
measurements\sidenote{At the end of operation (15:00), for the 8-hour horizon:
$T_{ts,h,t}=$96.98~$^\circ$C \vs~95.69 and $T_{ts,c,b}=$80.2~$^\circ$C \vs~81.7.
In terms of energy stored: $Q_{ts,c}=$194.45~kWh$_{th}$ \vs~211.36 and
...
clipped) and in the second part of the test where the inlet cooling water
temperature increases significantly, and since the flow was kept
constant in the test the condenser is not stable
anymore\sidenote{\reffig{solarmed:modelling:validation:solarmed} -- \textit{MED
