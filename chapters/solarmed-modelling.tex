...
models for each component of the \gls{solarmedLabel} system. A discrete
representation of the system is used, where the process variables are
sampled at a fixed rate, $T_s$, and the system is described by a set of
difference equations.

Even though this modelling component represents the dynamic behavior of the system,
some of the models described in the following sections are steady-state models.
...
These two dynamic parameters are calibrated using on experimental data. The
...
\begin{modelcounter}{Solar field}
    \begin{align*}
        T_{\text{out}}(k) &= \text{sf\:model}\left(
            ...
            H
        \right) \\
        & L_{pipe,eq} = \frac{T_s}{A_{pipe,eq}} \sum_{k=0}^{n} q_{sf}(k) \: \text{Description} \\
        & L_{eq} = n_{c,s} \cdot L_t \\
        & c_f = n_{c-loop} \cdot n_{tub-c} \\
        ...
        & T_{\text{out}}(k) = T_{\text{out}}(k-1) + (K_1 \cdot I - K_3 \cdot (T_{\text{out}}(k-1) - T_{\text{in}}(k-n_{d}) - K_2\cdot{\overline{T}})) \cdot T_s 
    \end{align*}
...
The main difference with respect to the model presented
    ...
    \begin{enumerate}
        ...
        average flow rate ($\overline{q_{sf}}$) across the delay samples
...
\reffig{\reffig{solarmed:modelling:sf:electrical_consumption_tests}} - 20250925
...
\begin{modelcounter}{Thermal storage electrical consumption}
    \begin{align*}
        ...
        & C_{e,ts} = 0.0001 \cdot q_{ts,src}^3 - 0.01 \cdot q_{ts,src}^2 + 0.5 \cdot q_{ts,src} + 1
    \end{align*}
