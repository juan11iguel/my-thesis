...
\begin{table*}[htbp]
\caption{Summary table of the prediction results obtained with the different
modelling approaches studied. \textbf{T} and \textbf{V} stand for training and 
...
\begin{margintable}[]
    \caption{Bounds and discretization of the model input variables. Lower and
    upper bounds are indicated as \textbf{lb} and \textbf{ub}, respectively. The
    number of discretization steps is indicated as \textbf{n}.}
...
$P_{w,s2}=80\times P_{e} = 8\pm5$~\euro/m$^3$ for source~2.

The thermal load profile is created by fixing the vapor temperature at
$T_v=35\,^\circ$C and generating an arbitrary cooling power based on the heat
available from a scaled down \gls{cspLabel}
model~\sidecite{ortegadelgado_theoretical_2016}. Finally, the maximum initial
available water volume is set to $V_{avail,0}=2$~m$^3$ (only available during
...
The cooling costs obtained in each scenario are shown in
