...
\tldrbox{%
This chapter presents the modelling and optimization results for the
combined cooling pilot plant. First, calibration and training results of various
modelling approaches are shown, followed by validation of the complete
integrated model. The best-performing data-driven model, the \gls{gprLabel},
is calibrated using synthetic data from the first-principle models. This
surrogate and on-demand model, which can be adapted to the particular case
study while remaining fast and computationally efficient, achieves a
\gls{maeLabel} below 0.97~$^\circ$C for temperatures and 19.4~l/h for water
consumption. In the second part of the chapter, the proposed optimization
strategies are compared. The horizon optimization alternative, which is also
validated experimentally, shows superior performance.%
}
...
As mentioned, the system consists of two main components, the \gls{wctLabel} and 
the \gls{dcLabel}. The different modelling approaches results are compared in
this section. The alternatives considered are:
...
\begin{marginfigure}[*-5]
    \includegraphics[width=1.1\linewidth]{cc-poppe-Me_vs_mwma.png}
...
\textit{\gls{wctLabel}-cal}), the Merkel performance number, $\Me$ can be fitted.
...
established according to the expected operating conditions--- as defined in
\reftab{cc:validation:wct:samples-params}.

\begin{margintable}[]
    \caption[Bounds and discretization of the model \gls{wctLabel} input
    variables]{Bounds and discretization of the model \gls{wctLabel} input
    variables. Lower and upper bounds are indicated as \textbf{lb} and
    \textbf{ub}, respectively. The number of discretization steps is indicated
    as \textbf{n}.}
    ...
    \begin{tabular}{lcccc}
        ...
        \midrule
        $T_{wb}$       & $^\circ$C            & 3   & 40   & 7 \\
        $\Delta T_{wb-in}$       & $^\circ$C            & 3   & 30    & 15  \\
        $T_{wct,in}$   & \%         & 25  & 45   & 10 \\
        $q_{wct}$       & m$^3$/h            & 6   & 24    & 12  \\
        $\omega_{\text{wct}}$  & \%         & 21  & 93.4   & 10 \\
        $\dot{m}_{wct}/\dot{m}_{wct,air}$  & -         & 0.5  & 4   & 10 \\
        \bottomrule
    \end{tabular}
    }
\end{margintable}
...
\begin{figure}[htbp]
    \includegraphics[width=.9\textwidth]{cc-validation-wct-regression.png}
...
\textit{\gls{wctLabel}-val}). In \reftab{cc:validation:wct:results}, the
performance of the studied modelling approaches is included for the different
performance metrics\sidenote{Described in \refsec{intro:modelling:metrics}}
where \texttt{T} represents the performance metric value  for the training /
calibration dataset and \texttt{V} for the validation one. 

In both the figure and the table, there are two Gaussian PR models: one trained
using the experimental data (labeled as \textit{Gaussian PR}) and the second one
corresponding to a surrogate model trained using the synthetic data from the
physical model (labeled as \textit{Gaussian PR (FP)}).

Comparing the physical model to the data-driven ones, three groups can be
identified: gradient boosting and random forest alternatives underperform, with
R$^2$ values around 0.8 and clear outliers in
\reffig{cc:validation:wct:regression}. \gls{annLabel} alternatives generally
provide good results, especially the radial-basis architecture. Finally, the
...
variables. This advantage is maintained even in the case of the surrogate model,
which reaches R$^2$ values of 0.94 and 0.95 for the outlet temperature and water
consumption, respectively. The physical model provides superior performance for
both output variables, obtaining an \gls{rmseLabel} of 0.33~$^\circ$C and an
R$^2$ of 0.98 for the temperature; and in terms of water consumption,
\gls{rmseLabel}~=~8.5~l/h and R$^2$~=~0.97. This advantage of the physical model
explains why the surrogate model is able to outperform many of the data-driven
alternatives, as it is a good approximation of the superior physical model.
Additionally, it achieves this at a fraction of the computational cost, with
vectorization support, a limited need for experimental data (the same as the
physical model), and coverage of a wide input space.

\begin{table*}[htbp]
\caption[Summary table of the \gls{wctLabel} prediction results obtained with
the different modelling approaches studied]{Summary table of the \gls{wctLabel}
prediction results obtained with the different modelling approaches studied.
\textbf{T} and \textbf{V} stand for training and validation datasets,
respectively.}
...
\begin{marginfigure}[]
    \includegraphics[]{cc-dc_nusselt_regression.png}
...
The following $\dot{m}_{dc,air}$-$\omega_{dc}$ relation was obtained from the
\reftab{cc:exp-campaigns} -- \textit{\gls{dcLabel}-fan} dataset.

\begin{equation}
    \dot{m}_{dc,air} =0.30195\cdot \omega_{dc}-1.02179. 
    \labeq{cc:dc:ma}
\end{equation}
...
\begin{equation}
    \Nus_a = 0.006411 \cdot \Rey_{air}^{0.9143} \cdot \Pra_{air}^{0.36}. 
    \labeq{dc:Nu}
\end{equation}
...
ranges established according to the expected operating conditions, as defined in
...
\begin{margintable}[]
    \caption[Bounds and discretization of the \gls{dcLabel} model input
    variables]{Bounds and discretization of the \gls{dcLabel} model input
    variables. Lower and upper bounds are indicated as \textbf{lb} and
    \textbf{ub}, respectively. The number of discretization steps is indicated
    as \textbf{n}.}
...
\begin{table*}[]
\caption[Summary table of the \gls{dcLabel} prediction results obtained with the different
modelling approaches studied]{Summary table of the \gls{dcLabel} prediction results obtained
with the different modelling approaches studied. \textbf{T} and \textbf{V} stand
...
The results of the different modelling alternatives for predicting the outlet
temperature can be seen in \reftab{cc:validation:dc:results} and
\reffig{cc:validation:dc:regression} (both using the validation dataset,
\reftab{cc:exp-campaigns} -- \textit{\gls{dcLabel}-val}). The table presents the
performance of the physical and data-driven approaches in terms of the selected
performance metrics\sidenote{Described in \refsec{intro:modelling:metrics}}.

\begin{figure}
    \savebox\captionqr{\qrcode[hyperlink,height=0.5in]{\repositoryBaseUrl/figures/cc-validation-dc-regression.html}}
    \includegraphics[width=\textwidth]{cc-validation-dc-regression.png}
...
Once again, two Gaussian Process Regression models are included: one trained
directly with the experimental data (\textit{Gaussian PR}) and the other acting
as a surrogate trained with the synthetic data from the physical model
(\textit{Gaussian PR (FP)}).
...
(\gls{rmseLabel}~=~0.50~$^\circ$C, \gls{maeLabel}~=~0.42~$^\circ$C, and
\gls{mapeLabel}~=~1.28~\%). Nevertheless, the Gaussian PR trained with
experimental data surpasses this performance in all metrics: it achieves R$^2$
values of 0.99 for both training and validation, and substantially lower error
indicators (\gls{rmseLabel}~=~0.24--0.32~$^\circ$C,
\gls{maeLabel}~=~0.19--0.25~$^\circ$C). The surrogate version, Gaussian PR (FP),
also delivers excellent results, nearly matching the experimental Gaussian PR
model with R$^2$ values close to 1.00 and very low errors
(\gls{rmseLabel}~=~0.03--0.53~$^\circ$C, \gls{maeLabel}~=~0.02--0.44~$^\circ$C).
...
values between 0.78 and 0.85. However, their errors
(\gls{rmseLabel}~≈~1.37--1.62 $^\circ$C and \gls{mapeLabel}~≈~3--4~\%) are
significantly higher than those of the Gaussian PR. Tree-based methods show
mixed outcomes: random forest underperforms with validation R$^2$~=~0.61 and
high prediction errors (\gls{rmseLabel}~=~2.17 $^\circ$C,
\gls{mapeLabel}~=~4.69~\%), while gradient boosting achieves nearly perfect
...
predictive accuracy with extremely low computational cost. The physical model,
although slightly less accurate than Gaussian PR, still offers solid results and
remains a valuable benchmark for validating the data-driven approaches.
...
% \begin{figure}
%     \includegraphics[width=\textwidth]{cc-validation-condenser-U.png}
...
calibration campaign (see \reftab{cc:exp-campaigns} --
\textit{\gls{scLabel}-cal}) were used to calculate the global heat transfer
coefficient, $U_{c}$, with \refeq{cc:sc_UA} together with the surface condenser
manufacturer's specifications from \reftab{cc:facility:sc}. $U_{c}$ is in this
case calibrated as a function of the input variables $T_{c,in}$ and $q_{c}$,
...
$p_1$=12.71, $p_2$=2.91, $p_3$=9.5$\cdot$10$^{-3}$, $p_4$=343.29,
$p_5$=-1$\cdot$10$^{-3}$ and $p_6$=-4.83$\cdot$10$^{-2}$, with each parameter,
$p_i$, has units consistent with the variables involved.


Using the previous relation and equations from \refmod{sc}, the surface
condenser model can be solved to estimate $T_{c,out}$ and $T_v$. Otherwise if
$T_v$ is provided $T_{c,out}$ and $T_{c,in}$ can be
...
results depicted in \reffig{cc:validation:condenser}. The predicted outlet water
temperature in the tubes ($T_{c,out}$) shows good agreement with the
experimental measurements, with a low \gls{maeLabel}. In contrast, the
prediction error is larger for the condensate water temperature ($T_{cond}$,
\gls{maeLabel}~=~1.86~$^\circ$C). This discrepancy may arise because the
condensation temperature obtained experimentally does not exactly match the
vapor saturation temperature ($T_v$), but is instead 0.1 to 1.7~$^\circ$C
subcooled.
...
First-principle models offer high accuracy and broad applicability, as they can
predict cooler operation under a wide range of conditions ---even scaled
systems--- without retraining. However, their main drawback is their long
execution time, which becomes limiting for optimization tasks requiring many
rapid evaluations. Data-driven models, in contrast, run orders of magnitude
faster with nearly constant runtime regardless of the input conditions and are
therefore well suited for vectorized evaluation. This speed, however, comes at
the cost of generality: they are restricted to the specific system and operating
conditions on which they were trained, and achieving robust performance
typically requires substantially larger datasets.
...
data from the first-principle models. This approach provides a way of having
on-demand models that can be adapted to the particular case study, while still
being fast and efficient in terms of computational resources.
...
\begin{itemize}
    \item The dashed circle represents the nominal cooling power (200~kW$_\text{th}$).
...
$\dot{m}_{air}-\omega_{wct}$ relationship at low flow rates. In the case of the
water consumption, the tendency (R$^2$=0.82) of the predicted values follows the
...
\begin{marginfigure}[-19cm]
    \includegraphics[width=\linewidth]{cc_model_regression.png}
...
\begin{figure*}
    ...
    \subfloat[\centering \gls{wctLabel}]{
        \includegraphics[width=0.9\linewidth]{cc-validation-samples_wct.png}
    }
    \\[1ex] % line break between subfigures, adjust spacing
    \subfloat[\centering \gls{dcLabel}]{
        \includegraphics[width=0.75\linewidth]{cc-validation-samples_dc.png}
    }
...
%================================
%================================
\section{Control and optimization results}[Control and optimization]
...
\nrefsec{cc:optimization}. First, an optimization algorithm is chosen from the
comparison between different alternatives for each optimization problem variant
(\ie static and horizon). Then, these two variants are compared for the combined
cooler, in simulation, for one representative operation period in the simulated
pilot plant in order to see which one performs better. Finally, the horizon
optimization variant is tested in the real facility. During the test, planned
...
For every static optimization problem (\refprob{cc:dc}, \ref{prob:cc:wct},
\ref{prob:cc:static}) three different algorithms are tested: \gls{seacstrLabel},
\gls{ihsLabel} and \gls{decstrLabel}. For each alternative the same number of
objective function evaluations ---\ie~budget--- is given (800) but they are
distributed differently depending on the algorithm:
\begin{itemize}
    \item \gls{seacstrLabel} and \gls{decstrLabel} make use of the
    \gls{cstrLabel} wrapper algorithm, which allows them to to solve constrained
    problems. 10 iterations are performed for this wrapper algorithm, leaving 80
    iterations to spare for the inner algorithm.
...
&  & \multicolumn{7}{c}{\textbf{Average fitness per obj. fun. evaluations} (u.m.)}
\\ \cline{5-9} \cline{11-17} 
...
evaluations\sidenote{Only up to 50k evaluations are shown in the figure for
...
\begin{figure*}[h!]
    \includegraphics[]{cc-validation-path_explorer_algo_comp.png}
...
Simulation results are presented to first analyze in detail how the horizon
optimization strategy operates the combined cooler system under different
environmental conditions. Then, a comparison between the static and horizon
optimization strategies is carried out to evaluate their performance in terms of
operational costs.

\textbf{Environment}. The optimization environment for this simulated analysis
is defined as follows. Hourly weather data is obtained from a \gls{tpyLabel} of
Tabernas (Spain) for the year~\sidecite{meteotestag_meteonorm_}. Electricity
cost data were taken from the 2022 Spanish market pool~\sidecite{REE_website}.
The water cost is defined as $P_{w,s1}=0.03$~\euro/m$^3$ for source~1, and
$P_{w,s2}=80\times P_{e} = 8\pm5$~\euro/m$^3$ for source~2. The thermal load
profile is created by fixing the vapor temperature at $T_v=35\,^\circ$C and
generating an arbitrary cooling power based on the heat available from a scaled
down \gls{cspLabel} model~\sidecite{ortegadelgado_theoretical_2016}. Finally,
the maximum initial available water volume is set to $V_{avail,0}=2$~m$^3$ (only
available during the peak precipitation period).

\subsubsection{Horizon optimization}

\reffig{cc:simulation:results-multiple-days} shows the optimization results
---in simulation and at pilot-scale--- for the horizon optimization
...
moderate temperature and water availability; and finally hot and dry. The
cooling costs obtained in each scenario are shown in
\reffig{cc:simulation:results-multiple-days} under \textit{Operational costs},
with electricity-related costs displayed in yellow and water-related costs in
green.

In terms of hydraulic distribution, it can be observed that when the water
resource is abundant (\ie 15-February) the system relies mostly on the wet
cooler. When combined with the dry cooler, this is the only scenario where the
parallel configuration is used, while for both in the moderate (15-June) and the
hot-dry (15-September) scenarios, the system operates mostly in series, or using
...
respect to the total ($\dot{Q}_{c}$). It can be observed that in the cold and
wet season the system relies mostly on the wet cooler with some minor assistance
from the dry cooler ---also boosted by the low ambient temperatures. In the
moderate scenario the roles reverse, with the dry cooler providing most of the
cooling power while the wet cooler is used only occasionally ---at the central
hours of the day--- where ambient temperatures are higher and the dry cooler is
not able to cope with the full load. Finally, in the hot and dry scenario, the
system tries to rely exclusively on the dry cooler. However, at the peak hours
of the day, the dry cooler is not able to provide the full cooling load, and the
wet cooler is used to supplement the cooling power (11:00 -- 16:00).

Comparing the costs, in the cold and wet season (15-Feb) cheaper cooling is
obtained compared to the other two scenarios\sidenote{Not zero but orders of
magnitude cheaper cooling is obtained} since the thermal load is lower and the
wet system can be broadly used. In the moderately hot and dry season significant
higher costs can be observed due to the extended use of the more expensive dry
cooler. This is still cheaper than the hot and dry season, where on top of the
already more expensive dry cooler cooling, the sparse wet cooler use and the
lack of water resource significantly increase the cooling costs by 50~\%
compared to the moderate scenario.
...
%     \makebox[\textwidth][c]{\includegraphics[angle=90,width=0.65\paperwidth]{cc-horizon-optimization-results-multiple-days.png}}%
% \end{figure}%
% \restoregeometry
% }
...
\begin{figure*}[h!]
    \includegraphics[]{cc-validation-comp_static_horizon.png}
...
A hierarchical control\sidenote{See \nrefsec{intro:hierarchical-control}}
strategy has been implemented in order to validate the optimization strategy in
the real facility. \reffig{cc:validation:implementation:diagram} shows a diagram
...
\nrefsec{cc:optimization:horizon}} and the right side shows the low-level
regulatory control layer, which directly interfaces with the actuators and
sensors of the facility. 

\begin{figure*}
    \includegraphics[]{cc-validation-diagram.png}
...
8\pm5$~\euro/m$^3$ for source~2. Finally, the initial available water volume was
set to $V_{avail,0}=0.4$~m$^3$, after which it was updated dynamically according
to the system's real-time consumption data.
...
\textbf{Optimization layer}. The optimization algorithm is run periodically,
...
variables are shown in \reftab{cc:validation:optimization-bounds}. 

% Since the low-level control layer actuates over the coolers fan speeds, instead
% of using $\omega_{dc}$ and $\omega_{wct}$ directly as setpoints, the
% corresponding outlet temperatures values ($T_{dc,out}$ and $T_{wct,out}$) are
% used instead. This makes the strategy more robust, since as long as the outlet
% temperatures from the coolers are kept, and they will be since the fan speed is
% continuously regulated to achieve that, the correct condenser operation can be
% guaranteed, despite possible deviations. 
...
Classical feedback loops with \gls{piLabel} have been used in this regulatory
layer, which were tuned using the improved SIMC technique~
\sidecite{skogestad_simc_2012}. \reftab{cc:validation:control} shows the
proportional gain, $K_p$, and the integral time, $T_i$, for each control loop.
...
a 3$\times$3 matrix of regions has been defined to account for three levels:
low, medium and high values in the three main variables involved
...
strategy: as long as the outlet temperatures of the coolers are maintained
---and they are, since fan speeds are continuously regulated to achieve them---
the proper operation of the condenser can be ensured, even in the presence of
potential deviations due to modelling inaccuracies, external disturbances or
uncertainty in the forecasted weather conditions.
...
\reffig{cc:validation:exp-results} visualizes one test carried out the 1st
August. The objective of the test was twofold. For the first part of the test a
set operation plan was established: 
...
The objective was to validate that the optimized operation ---using the
available environment prediction--- was effective at correctly managing
resources and cool the thermal load with the expected consumptions. During a a
\gls{cspLabel} plant operation, changes to the operation plan can arise in
response to changes in electricity market dynamics, or other unforeseen
environment circumstances. At 10:20 a change is introduced in the operation plan
to simulate this behavior. The thermal load was reduced with a similar (inverse)
profile to the initial one. This allows to verify the adaptability of the
proposed strategy to changing conditions which is the second objective of the
test.

The operation strategy was as follows:

\begin{itemize}
    \item The low-level control layer is activated since the beginning of the test.
    ...
    initial perspective on the day operation and expected consumptions. These
    values were used as reference and manually set to bring the system into
    stable operation after gradually increasing the thermal load.
    
    \item The optimization sample time was 20 minutes and is evaluated every 40
    minutes.
    ...
    \item For each optimization layer evaluation, the environment is first
    updated and then provided as input to the optimization process (see
...
In \reffig{cc:validation:exp-results} -- \textit{Hydraulic Distribution},
several sequentially added bars are shown. The first bar corresponds to the
experimental value, while the remaining bars represent the predicted hydraulic
distributions from successive optimization evaluations: the second bar comes
from the first evaluation, the third from the second, and so on.
...
\begin{itemize}
    ...
    is valid for hours. Particularly, the initial evaluation at 08:30 is valid
    until the introduced change at 10:30\sidenote{In another test (not shown
    here) where the thermal load did not change throughout the day and the
    initial optimization held valid until the end of operation}. As mentioned,
    in this particular test a planned change in the thermal load profile is
    introduced at 10:30, which is then taken into account in the optimization
    layer re-evaluation at 10:40, and thus a new adapted operation strategy to
    the new scenario is generated and successfully applied.
    
    \item The dry system is highly sensitive to ambient temperature when
    operating near its limits. A prediction error of less than half a degree
    (0.4~$^\circ$C between 09:04 and 09:17) results in a 15~\% deviation between
    the expected and actual fan speed.

    
    \item When both systems are operating, the wet cooler can compensate for any
    shortfall in the dry cooler's assigned cooling duty. However, as observed at
    the beginning of the test (09:04--09:17), when only the dry cooler is active
    and operating at its limits (\reffig{cc:validation:exp-results} --
    \textit{\gls{dcLabel} outlet temperature loop -- Control signal}), the load
    may become undercooled. This leads to an increase in condenser pressure,
    with outlet temperatures rising by approximately 1--2~$^\circ$C, which would
    penalize the turbine's power production. To avoid such situations, a
    low-level supervisory controller should be implemented\sidenote{This is
    indicated in \reffig{cc:validation:implementation:diagram} --
    \textit{Regulatcory control} -- \textit{Cooling power}}.

    
    \item To avoid using the more expensive alternative water source, the
    optimization prioritized the use of the dry cooling (as far as being
    dry-only as long as the ambient temperature and demanded thermal load
    allowed it, up until 09:20) to conserve water until the end of operation.
    After the operation plan changes, the lower expected load gives more room for
    adjustment and the optimization increases the load through the wet system
    from 0--40~\% to about 50~\% (\reffig{cc:validation:exp-results} --
    \textit{Hydraulic distribution and Cooling power distribution}).
    ...
    receives an operation strategy for the entire horizon in the form of a
    vector of setpoints. This allows the controller to remain robust even if the
    optimization is not recomputed or updated on time, as it can continue
    applying the previously evaluated setpoints until new ones become available.
    And, as observed, the operation strategy remains valid for long periods
    ---in the order of hours--- as long as unexpected changes in the environment
    do not occur.
\end{itemize}
