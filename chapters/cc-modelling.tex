...
\tldrbox{
    This chapter describes modelling of the different components of a combined
    cooling system, mainly a \gls{wctLabel} and a \gls{dcLabel}. Different
    alternatives are presented, including the generation of samples for
    ...
    shown how they are integrated into a complete system model.
}
...
Aquí describir los rangos en los que se han movido las variables de entrada y
el criterio para elegir esas variable de entrada para construir la combinatoria
de valores de entrada.

Finally, all valid thermodynamic and operational combinations are merged into a comprehensive sample set, enabling detailed system evaluations across a realistic and constrained input space.
...
% TODO: Esto es metodología, mover a cc:modelling y aquí solo comentar la tabla
% de muestreos seguida y mostrar la gráfica de resultados
In this work, sample generation is carried out using a structured grid-search
approach to ensure comprehensive coverage of the input space. First, the most
relevant input variables for the system are identified, and ranges are
specified based on expected operating conditions. Then, the input space is
discretized using a fixed number of resolution steps for each variable, as
defined in Table .... Instead of generating values for the inlet temperature
input variable, values are generated for the temperature difference with the
dry-bulb temperature, therefore, a 2D grid is
constructed using combinations of ambient temperature (Tamb) and the difference
between inlet and ambient temperature (deltaTamb-Tin), with infeasible
combinations filtered based on physical constraints. For each valid temperature
pair (Tamb, Tdcin), additional independent variables (qdc, wdc) are combined
via a Cartesian product, resulting in a full multidimensional
grid of plausible operating points. This systematic procedure ensures a dense
and uniform sampling across all relevant input dimensions
...
\end{modelcounter}
\marginnote[*-2]{$p_i$ are the coefficients of the polynomial regression. They need to be calibrated for each component.}

where \(C_e\) is the electrical consumption and \(x\) is the input variable
(\eg the recirculated cooling water flow rate, the dry cooler fan speed, etc.).

%================================
\subsection{Surface condenser}
\labsec{cc:modelling:surface-condenser}
...
\marginnote[*-2]{The audacious users might feel tempted to edit some of 
...
The complete model of the combined cooling system is based on the integration
of the models of the \gls{wctLabel} and the \gls{dcLabel} with the surface
condenser and the mixers, and defined in \nrefmod{cc}.\sidenote{The cooling
water recirculation electrical consumption is associated with the condenser
here but not all consumption can be attributed to the condenser, the hydraulic
circuit and specially the dry cooler will also offer a significant circulation
resistance}
...
\begin{modelcounter}{Combined cooler}
    \begin{align*}
        T_{cc,out},\,C_{e}&,\,C_{w},\,T_{c,out} = \text{combined\:cooling\:system\:model}(q_{c}, R_{p}, R_{s}, \omega_{dc}, \omega_{wct},T_{amb},HR_i,T_{v},\dot{m}_{v}) \\
        & T_{cc,in}=T_{c,out} \\
        & T_{dc,in}=T_{cc,in} \\
        % three-way valves
        ...
        & C_{e,dc} = \text{electrical\:consumption}(q_{dc}) \\
        & C_{e,wct} = \text{electrical\:consumption}(q_{wct}) \\
        % totals
...
Finally, the complete model diagram with all its variables can be seen in \reffig{cc:modelling:complete-model-diagram}.
