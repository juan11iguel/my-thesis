...
\tldrbox{ This chapter describes the steady-state modelling of the different
    components of a combined cooling system. Different alternatives are
    presented: from physical models to data-driven approaches, including the
    generation of surrogate data-driven models trained using synthetic data from
    a physical model. Finally, it is shown how they are integrated into a
    complete system model.\\[1ex]

    \begin{minipage}{\linewidth}
        \centering
        \includegraphics[width=\linewidth]{cc-modelling-complete-model-diagram-with-bg.png}\\
...
state modelling of the combined cooler main components, \ie the
\fullgls{wctLabel}, the \fullgls{dcLabel} and the \fullgls{scLabel}. More
specifically, the aim is to develop two modelling strategies: that based on
physical equations and that based on black box data-driven models. 

First, in \refsec{cc:modelling:wct} a physical model for the \gls{wctLabel} is
described followed by a methodology to generate a synthetic dataset for
data-driven surrogate models. In \refsec{cc:modelling:dc} the same structure is
followed for the \gls{dcLabel}-type \gls{acheLabel}. Other first-principles
model components are presented in \refsec{cc:modelling:other-components}: the
surface condenser, mixers, and the electrical consumption. Finally, all
components are integrated in \refsec{cc:modelling:complete-model} to form the
complete model of the combined cooling system. The individual component and the
\gls{ccsLabel} model sections are completed with a model interface block. 
...
\vspace{1.5cm} 
\begin{figure}[htbp] 
    \begin{overpic}[width=.7\textwidth]{poppe_control_volume.pdf} 
        \put(-3.3,26){$dz$}
    ...
    \end{overpic} 
    \vspace{1cm} 
    \caption[Control volume in the exchange area of a wet cooling tower
    arrangement]{Control volume in the exchange area of a wet cooling tower
    arrangement. Variables are described in the Nomenclature section.\\Source:
    Serrano \etal~\cite{serrano_wet_2024}.}
...
\marginnote[*-5]{$Le$ is the Lewis number, $\omega$ is the humidity ratio
(kg/kg), $h$ is the enthalpy and $c_p$ the specific heat.}

where the evolution of key variables such as the humidity ratio
(\refeq{cc:poppe_omega}), air enthalpy (\refeq{cc:poppe_h}), and the Merkel
...
%=====================================
\subsection{Generating synthetic data for surrogate \gls{wctLabel}
Model}[Surrogate model]
\labsec{cc:modelling:wct:samples}

The first pair of input variables for the \gls{wctLabel} synthetic dataset
generation are the wet bulb temperature ($T_{wb}$) and the difference between
this temperature and the system inlet temperature ($\Delta T_{wb-in}$). The wet
bulb temperature ---the most relevant thermodynamic variable for the
\gls{wctLabel} performance--- is used instead of the ambient temperature or the
relative humidity.
Using both the ambient temperature and the relative humidity would lead to a
larger than necessary input space with many duplicate samples, as the wet bulb
temperature is a function of both variables. The second pair of input variables
are the cooling water flow rate ($q_{wct}$) and, following the reasoning from
the physical model, the air to water mass flow ratio
($\dot{m}_{wct,air}/\dot{m}_{wct}$), since it is a key parameter in defining the
operating conditions of the tower. From the resulting 2D grid, valid
combinations are obtained by calculating the air mass flow rate and finding if a
valid fan speed can be obtained using an air mass flow rate to fan speed
...
coefficient and the logarithmic mean temperature difference.
 ...
 \begin{align}  
    & \dot{Q}_{dc, released}= \dot{m}_{dc} \cdot c_{p,w} \cdot(T_{dc,in}-T_{dc,out}), \label{eq:cc:dc_released} \\ 
    & \dot{Q}_{dc, absorbed} = \dot{m}_{dc,air} \cdot c_{p,air}\cdot(T_{dc,air,out}-T_{amb}), \label{eq:cc:dc_absorbed}\\ 
...
passes~\sidecite[*-1]{kroger2004}, and the product of the overall heat transfer
...
\begin{equation}\labeq{cc:Nu_corr}
    \Nus_a = G \cdot \Rey_{air}^{m} \cdot \Pra_{air}^{0.36}, \\
\end{equation}
...
by Zukauskas for staggered tube banks~\sidecite{kakac}\sidenote{ The system
design characteristics needed for applying this methodology for
the pilot plant are listed in \reftab{cc:facility:dc}. }.

%=====================================
\subsection{Generating synthetic data for surrogate \gls{dcLabel}
model}[Surrogate model]
...
\begin{modelcounter}{Dry cooler}
    ...
    \labmod{dc}
\end{modelcounter}
\marginnote[*-6]{$\theta_{dc}$ are the model parameters such as the area ($A_{dc}$), that can be found in \reftab{cc:facility:dc}.}
...
\begin{modelcounter}{Surface condenser}
    ...
    \labmod{sc}
\end{modelcounter}
\marginnote[*-10]{$\theta_c$ are the model parameters such as the condenser area ($A_c$), that can be found in \reftab{cc:facility:sc}.}

Being $\dot{m}_c$ the condenser cooling water flow rate, $T_{v}$ the vapor
temperature inside the condenser and $\dot{m}_v$ its flow rate. $U_c$, the
overall heat transfer coefficient, can be determined in a variety of ways. One
of them is given by:
\begin{equation}
...
once again using the Nusselt method~\sidecite{Serth2007} for laminar-flow
condensation over a horizontal tube bundle ($0 < \Rey < 30 \times 10^{6}$)
including the Kern correction for condensate inundation, while the tube-side
convective heat transfer coefficient for the water flow inside the tube bundle,
$h_{i}$, is approximated using the Petukhov-Kirillov-Popov correlation for
fully-developed turbulent flow through smooth circular
tubes~\sidecite{rohsenow_handbook_1998}, valid within $0.5 < \Pra < 10^{6}$ and
$4000 < \Rey < 5 \times 10^{6}$.

%================================
\subsection{Electrical consumption}

For each component (\eg, the recirculated cooling water flow rate,
cooler fan speed, etc.). electrical consumption is modelled with polynomial
regressions of order 3 derived from experimental data:
...
\end{modelcounter}
\marginnote[*-4]{$\theta$ represents the model parameters, \ie, the polynomial
coefficients ($p_1, p_2, p_3, p_4$).}

where \(C_e\) represents the electrical consumption, and \(x\) is the input
variable. The coefficients \(p_i\) correspond to a polynomial regression
and must be calibrated individually for each component.

\subsection{Mixers}

Fluids are mixed at the inlet of the \gls{wctLabel} and at the outlet of the
combined cooler (\texttt{MIX~1} and \texttt{MIX~2} in
\reffig{cc:modelling:complete-model-diagram}, respectively). The mixers outlet
flow ($q_{mix,out,i}$) and temperature ($T_{mix,out,i}$) can be determined with
a simple mass and energy balances from its inlets streams
($q_{mix,in},\,T_{mix,in}$):
...
The two divergent three-way valves (see
\reffig{cc:modelling:complete-model-diagram}) are modeled using the distribution
ratio $R$ as inputs, so the mass flow rate at the outlets of each valve can be
easily estimated as $q_{out,1}=R\cdot q_{in}$ and conversely
$q_{out,2}=(1-R)\cdot q_{in}$. Where $R$ is a value in the range [0,~1], which
implicitly includes the non-linearities of the valve.
...
\begin{figure}
    \includegraphics[width=.9\textwidth]{cc-modelling-complete-model-diagram.png}
...
mixers, as defined in \nrefmod{cc}. The full diagram, including all variables,
is shown in \reffig{cc:modelling:complete-model-diagram}.

% \sidenote{The electrical consumption for the
...
$\omega_{dc}$, $\omega_{wct}$, $T_{amb}$, $HR$, $T_{v}$, $\dot{m}_{v}$) as
follows. First, the flow rates through the different paths are determined from
the recirculation flow ($q_{c}$) and the valve positions ($R_{p}$, $R_{s}$).
Next, the condenser model is evaluated, yielding the remaining input needed to
fully specify the dry cooler, namely its inlet temperature
($T_{dc,in} = T_{c,out}$). The inlet and outlet temperatures for the wet cooler
are then obtained by evaluating the mixers using the dry-cooler outlet
temperature ($T_{dc,out}$) together with the combined-cooler outlet temperature,
which equals the condenser inlet temperature ($T_{cc,out} = T_{c,in}$). Finally,
the wet cooling tower is evaluated: given its inlet temperature
($T_{wct,in}$, from the first mixer) and fan speed ($\omega_{wct}$), its outlet
temperature ($T_{wct,out}$) and water consumption ($C_{w,wct}$) are obtained.
A consistency check is performed requiring that the wet-cooler outlet
temperature matches the value obtained from the second mixer.
...
\marginnote[*3]{$\theta_{cc}$ represents the parameters of the different
components (condenser area, heat transfer coefficients, number of tubes in the
\gls{acheLabel}, etc.)}

\begin{marginfigure}[*8]
    \centering
    \includegraphics[]{cc-modelling-wct-io-diagram.png}
    % {\footnotesize \textbf{(a)} \gls{mimoLabel} configuration\\}
    
    \vspace{1ex}

    \includegraphics[]{cc-modelling-dc-io-diagram.png}
    % {\footnotesize \textbf{(a)} \gls{mimoLabel} configuration\\}
    
    \vspace{1ex}
    
    \includegraphics[]{cc-modelling-c-io-diagram.png}
    % {\footnotesize \textbf{(b)} Cascade configuration}
    
    \caption{Inputs-outputs block diagram of the main model components}
    \labfig{cc:modelling:component-models-diagrams}
\end{marginfigure}

\begin{modelcounter}{Combined cooling system}
    \begin{align*}
        T_{cc,out},\,C_{e}&,\,C_{w},\,T_{c,in},\,T_{c,out} = \text{ccs\:\:model}(q_{c}, R_{p}, R_{s}, \omega_{dc}, \omega_{wct},T_{amb},HR,T_{v},\dot{m}_{v};\,\theta_{cc}) \\
        & T_{cc,in}=T_{c,out} \\
        & T_{dc,in}=T_{cc,in} \\
        % three-way valves
        ...
        % condenser model
        & T_{c,in},\,T_{c,out} = \text{condenser\:model}(q_c,\, \dot{m}_v,\, T_v;\,\theta_{c}) \\
        % dc model
        & T_{dc,out},\,C_{e,dc} = \text{dc\:model}(q_{dc},\, \omega_{dc},\, T_{amb},\, T_{dc,in};\,\theta_{dc}) \\
        % first mixer
        & q_{wct},\,T_{wct,in} = \text{mixer\:model}(q_{wct,p},\,T_{cc,in},\, q_{wct,s},\, T_{dc,out}) \\
        % final mixer
        & q_{cc},\,T_{cc,out} = \text{mixer\:model}(q_{wct},\, T_{wct,out},\, q_{dc},\, T_{dc,out}) \\
        % wct model
        & T_{wct,out},\,C_{e,wct},\,C_{w,wct} = \text{wct\:model}(q_{wct},\,\omega_{wct},\, T_{amb},\, HR,\, T_{wct,in};\,\theta_{wct}) \\
        % electrical consumptions
        & C_{e,c} = \text{electrical\:consumption}(q_c;\,\theta_{c,e}) \\
        & C_{e,dc} = \text{electrical\:consumption}(\omega_{dc};\,\theta_{dc,e}) \\
        & C_{e,wct} = \text{electrical\:consumption}(\omega_{wct};\,\theta_{wct,e}) \\
        % totals
        & C_{e} = C_{e,dc} + C_{e,wct} + C_{e,c};\,C_{w} = C_{w,wct}
    \end{align*}
    \labmod{cc}
\end{modelcounter}
