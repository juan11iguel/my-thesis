...
\reffig{solarmed:process-diagram}: a flat plate collector solar field which is
...
The parameters are the collector aperture
($A_c\,(m^2)$), number of parallel collectors in each loop ,number of serial
connections of collectors rows ($n_s$), number of parallel tubes in each
collector ($n_t$) and the length of the collector inner tube ($L_t$). Their
values are shown in \reftab{}.
...
% Nombre & Definición \ \hline
% $A_c$ & Área superficial de un captador (2.02 $\mathrm{m^2}$)\
% $A_e$ & Área superficial de espejo reflector\
% & que afecta a un captador (1.73 $\mathrm{m^2}$)\
% $C_p$ & Capacidad calorífica del agua ($\mathrm{J/(kg\cdot K)}$) \
% $d$ & Retardo (s)\
% $\mathrm{\dot{m_t}}$ & Flujo másico que circula por el tubo absorbedor\
% & de un captador ($\mathrm{kg/s}$) \
% $h_1$ & Coeficiente de pérdidas térmicas globales \
% & (3.523 $\mathrm{W/(m^2\cdot K)}$)\
% $h_2$ & Coeficiente de pérdidas térmicas globales\
% & (0.015 $\mathrm{W/(m^2\cdot K^2)}$)\
% $I_g$ & Radiación solar global \
% & en plano inclinado 35 $\mathrm{^o}$ ($\mathrm{W/m^2}$)\
% $I_n$ & Radiación solar directa normal ($\mathrm{W/m^2}$)\ $I_r$ & Radiación solar reflejada ($\mathrm{W/m^2}$)\
% $P_{loss}$ & Pérdidas térmicas ($\mathrm{W}$)\
% $P_I$ & Potencia radiante sobre el captador ($\mathrm{W}$)\
% $t$ & Tiempo (s) \
% $T_a$ & Temperatura ambiente ($\mathrm{^oC}$)\
% $T_{in}$ & Temperatura de entrada del agua ($\mathrm{^oC}$)\
% $T_m$ & Temperatura media del agua ($\mathrm{^oC}$)\
% $T_{out}$ & Temperatura de salida del agua ($\mathrm{^oC}$)\

%     $V$ & Volumen de agua en un captador (0.0022 $\mathrm{m^3}$) \\
%     $\beta_1$ & Factor de pérdidas ópticas medio \\
%         &(transmisividad y absortividad del captador, \\
%         & y ángulo de incidencia)\\
%     $\beta_2$ & Factor de pérdidas ópticas medio\\
%         &(reflectividad del espejo y ángulo de incidencia)\\      
%     $\rho$ & Densidad del agua ($\mathrm{kg/m^3}$) \\
%     $\theta$ & Ángulo de incidencia en el reflector\\
% 	\hline
% \end{tabular}
% \end{table}
...
\textit{Thermal storage}} consists on a two-tank system designed to ensure
thermal stratification. It has a total capacity of 40 m$^3$. One of the tanks
...
hot water within the XXX project in 2005. An image of the facility in its current state can be seen in
\reffig{solarmed:facility:med}It has been operated in different
experimental campaigns and configurations robustly for more than two decades,
as can be seen in \reffig{solarmed:tests-calendar}, which shows the operation
history of the plant (starting from 2009). The campaign from 2009 to 2012
focused on ... \sidecite{patricia} while the campaign from 2015 to 2016....
Finally, within the research work presented in this thesis, a new campaign was
performed to validate a standardization methodology proposal and experimentally
characterize the behaviour of the system at higher temperatures. This is
...
\begin{marginfigure}[] % -5.5cm
    \includegraphics[]{figures/med_test_days.png}
