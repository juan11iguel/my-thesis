...
the (N+1)-ES Simple Evolutionary
Algorithm\sidenote{\texttt{sea\_pop\_1\_gen\_1000} in
...
%
%
% \todo{Recordar el objetivo de la optimización (maximizar beneficio o minimizar costes de producción)}
%
%
%
%
...
length representing the active period duration\sidenote[][*-5]{Also displayed
...
the \gls{medLabel} operates for significantly shorter periods (\eg, 6.9~hours on Day~27,
...
the \gls{medLabel} at lower temperatures and for longer durations (9.8~h vs. 8.1~h, see
...
does not translate into higher benefits (see
...
temperature span between the Heuristic and optimized strategies\sidenote{See
\reffig{solarmed:optimization:results:schedule_comparison}}: 2.34~K for the
Heuristic versus 3.51~K (\gls{nlpLabel}) and 3.32~K (n\gls{nlpLabel}). This
indicates that maximizing \gls{solarmedLabel} system performance is achieved by
operating for longer periods at lower temperatures and with high temperature
differences--- in both the solar field and \gls{medLabel} sides--- thereby
maximizing the utilization of sensible heat.
...
\begin{figure}[t]
    ...
    \caption[]{Daily and total cumulative benefits comparison. \textit{u.m.} represents arbitrary monetary units.}
