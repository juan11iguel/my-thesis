...
Programme}~\sidecite{cnr_mediterranean_2024}. The figure also displays the
results for the alternative operation strategies across selected variables;
however, a detailed comparison of these strategies is presented in the next
section.
...
\reminder{Optimization objective and alternatives}{ 
    The aim is to define an operational strategy
    that manages both subsystems autonomously and efficiently and seeks to
    maximize water production while minimizing electrical consumption.\\
    ...
    benefit ($B$) is simply the negative of the operating cost. \\

    The operation strategies evaluated are: \\
...
strategy is compared on a day-by-day basis\sidenote[][*-5]{In
...
length representing the active period duration\sidenote[][*-10]{Also displayed
...
temperature span between the Heuristic and optimized strategies: 2.34~K for the
Heuristic versus 3.51~K (\gls{nlpLabel}) and 3.32~K (n\gls{nlpLabel}) ---see
